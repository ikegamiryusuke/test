...
数字はダイヤルを上下にスワイプして入力します。正しい 4 桁が揃うとスタンプが押され、紙吹雪アニメーションとともに枠が光ります。すべてのスポットを取得すると画面に「COMPLETE!!」が表示され、画面全体で紙吹雪が舞います。

シートの列名も `spot1`, `spot2` ... と揃えれば任意の数に拡張可能です。GAS 側の `Code.gs` ではヘッダー行を解析し、存在する `spot?` 列の数に応じて自動的に処理するため、列を追加するだけで拡張できます。

## スマホ表示最適化
このサンプルはスマートフォンでの利用を前提に、画面サイズに応じてレイアウトが自動調整
されるように作られています。`meta viewport` の設定と CSS のメディアクエリにより、
小さな画面でも操作しやすいボタン配置やフォントサイズになります。PC から閲覧した場合は
要素が大きく表示されますが、そのまま操作することも可能です。
