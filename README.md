...
このリポジトリは Google Apps Script (GAS) と Google スプレッドシートを利用した簡易チャットボットの実装例です。スプレッドシートに記入した知識をもとに Gemini API へ質問を送り、回答を返します。Python などは使用しません。

## 構成
- **Code.gs**: GAS のメインスクリプト。`doPost()` をエントリーポイントとして Web アプリ化します。
- **Google スプレッドシート**: 知識ベースを記入するシート。
- **Gemini API**: シートの内容と質問をもとに回答を生成します。

## セットアップ
1. 新しい Apps Script プロジェクトを作成し、`Code.gs` の内容を貼り付けます。
2. スクリプトプロパティに以下を設定します。
   - `GEMINI_API_KEY`: Gemini API キー
   - `KNOWLEDGE_SHEET_ID`: 知識ベースとして利用するスプレッドシートの ID
3. シートの共有設定を「リンクを知っている全員が閲覧可」に変更します。
...
## トラブルシューティング
- **応答が返ってこない**: スクリプトエディタの [実行数] 画面でログを確認し、エラーメッセージを参照します。API キーやシート ID が正しいかもチェックしてください。
