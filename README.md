...
## カスタマイズ
`main.js` の `SPOTS` 配列を書き換えるだけで、スタンプスポットの数やコード、画像 URL を自由に変更できます。ページ読み込み時に `SPOTS` からスタンプ枠を自動生成するため、HTML を個別に編集する必要はありません。`name` フィールドは枠の下に表示されます。シートの列名も `spot1`, `spot2` ... と揃えれば任意の数に拡張可能です。
